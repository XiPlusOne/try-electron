...
const RodCreator = ({
  ...
  const paintRod = (canvas: HTMLCanvasElement): void => {
    /* eslint-disable */
    ...
    const firstSectionLeftLineEnd: Coordinate = {
      x: firstSectionLeftLineStart.x + Math.sin(radian) * rodFirstSectionLength,
      y: firstSectionLeftLineStart.y + Math.cos(radian) * rodFirstSectionLength
    };
  ...
  class Rod extends React.PureComponent {
    props: Props;
    ...
    componentDidMount() {
      paintRod(this.refCanvas.current, this.props);
    }

    onTransitionEnd() {
      const { isRodOn, rodOnCallback, rodOffCallback } = this.props;

      if (rodOnCallback && isRodOn) {
        rodOnCallback();
      }

      if (rodOffCallback && !isRodOn) {
        rodOffCallback();
      }
    }
